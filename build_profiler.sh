...
# Build flags
CPP="-DCMAKE_CXX_COMPILER=g++"
FLAGS="-DPERF_PROFILING=ON"
...
cmake $CPP -DCMAKE_BUILD_TYPE=Release -DMARGOT_CONF_FILE=$CONF_NAME .. || exit -1
make -j4 || exit -1

# Compile application itself
mkdir -p $APPLICATION_ROOT/perf-build || exit -1
cd $APPLICATION_ROOT/perf-build
cmake $CPP  $FLAGS -DCMAKE_BUILD_TYPE=Release .. || exit -1
make -j4 || exit -1
