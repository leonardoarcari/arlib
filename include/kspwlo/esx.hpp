...
esx(PropertyGraph &G, Vertex s, Vertex t, int k, double theta) {
  // P_LO set of k paths
  ...
  auto heuristic = kspwlo_impl::distance_heuristic(G, t);
  ...
  using EdgePriorityQueue = std::priority_queue<Priority, std::vector<Priority>, kspwlo_impl::EdgePriorityComparator<Edge>>;
  auto edge_priorities = std::vector<EdgePriorityQueue>(k);

  // We keep a set of non-removable edges
  dnr_edges = std::unordered_set<Edge, hash<Edge>>{};

  // We keep a set of deleted-edges
  deleted_edges = std::unordered_set<Edge, hash<Edge>>{};
}
}
